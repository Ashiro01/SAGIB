          ...
          <v-row>
            ...
            <v-col cols="12" md="6">
              ...
              <v-text-field
                ...
                label="Código Patrimonial (Nomenclatura)"
                :rules="[rules.required]"
                required
                outlined
                dense
                :disabled="isEditing"
                hint="Este código debe ser único y no se puede cambiar después de crear."
              ></v-text-field>
...
export default {
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.bien = { 
          id: null, codigo_anterior: '', descripcion: '', marca: '', modelo: '', serial: '',
          cantidad: 1, fecha_adquisicion: null, fecha_adquisicion_picker: null,
          n_orden_compra_factura: '', proveedor: '', valor_unitario_bs: null, valor_unitario_usd: null,
          unidad_administrativa_actual_id: null, ubicacion_fisica_especifica: '', responsable_asignado_nombre: '',
          responsable_asignado_cargo: '', estado_bien: 'NUEVO', observaciones: '',
  ...
  async mounted() {
    this.cargarUnidadesParaSelector();
    const bienId = this.$route.params.id;
    if (bienId) {
      this.isEditing = true;
      this.cargarBienParaEditar(bienId);
    } else {
      this.resetForm();
    }
  },
