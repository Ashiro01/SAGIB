from rest_framework import viewsets, permissions, status # Añade status
from rest_framework.response import Response # Añade Response
from rest_framework.views import APIView
from django.db.models import Sum, Count, Max, F, Subquery, OuterRef
from django.db import transaction  # <-- AÑADIR ESTA IMPORTACIÓN
import pandas as pd
from .models import Bien, MovimientoBien, DepreciacionMensual # Añade MovimientoBien
from .serializers import BienSerializer, MovimientoBienSerializer # Añade MovimientoBienSerializer
from django.utils import timezone # Añade timezone
...
class BienViewSet(viewsets.ModelViewSet):
    ...
    # Configuración de Permisos (quién puede acceder a esta API)
    # Por ahora, permitiremos a cualquier usuario autenticado.
    # Más adelante podemos hacer permisos más granulares.
    permission_classes = [permissions.IsAuthenticatedOrReadOnly] 
    # IsAuthenticatedOrReadOnly: Usuarios anónimos pueden leer (GET), 
    # pero solo usuarios autenticados pueden escribir (POST, PUT, DELETE).
    # Para un control más estricto, podrías usar [permissions.IsAuthenticated]
    # o crear permisos personalizados.

    # Aquí podríamos añadir lógica personalizada si fuera necesario,
    # como generar el 'codigo_patrimonial' antes de guardar.
    # def perform_create(self, serializer):
    #     # Ejemplo: Lógica para generar código patrimonial antes de guardar
    #     # instance = serializer.save()
    #     # instance.codigo_patrimonial = f"IPSFA-BM-{timezone.now().year}-{instance.id:05d}"
    #     # instance.save()
    #     serializer.save() # Por ahora, guardado simple
...
class MovimientoBienViewSet(viewsets.ModelViewSet):
    ...
    #     return Response({"detail": "La actualización de movimientos no está permitida."}, status=status.HTTP_405_METHOD_NOT_ALLOWED)

    # def destroy(self, request, *args, **kwargs):
    #     return Response({"detail": "La eliminación de movimientos no está permitida."}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
